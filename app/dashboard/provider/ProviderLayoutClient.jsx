...
import {
  LayoutDashboard, ClipboardList, UserCircle2, CheckCircle2, LogOut
} from "lucide-react";
      ...
      <SidebarInset className="bg-[#f7f7f8] overflow-y-auto [scrollbar-gutter:stable]">
        ...
        <div className="hidden md:flex sticky top-0 z-30 h-16 shrink-0 items-center border-b border-zinc-100 bg-white/90 px-8 backdrop-blur-lg shadow-sm transition-all duration-200">
          ...
          <div className="flex-1 flex items-center justify-center">
            <SmartSearch role="provider" compact className="w-[360px]" />
          </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-zinc-950 border-t border-white/[0.06]">
        <div className="flex items-center justify-around h-16 px-2">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.name} href={item.href}
                className={`flex flex-col items-center gap-1 flex-1 py-2 rounded-md transition-colors ${
          ...
          <button onClick={handleLogout}
            className="flex flex-col items-center gap-1 flex-1 py-2 rounded-md text-red-400 hover:text-red-300 transition-colors"
          >
